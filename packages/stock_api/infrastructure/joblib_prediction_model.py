...
class JoblibPredictionModel(PredictionModel):
    def __init__(self, model_path: str):
        ...
        logger.debug(
            ...
            self._booster, self._scaler, len(self._columns),
        )
    ...
    def _extract_features(self, text: str, company_name: str) -> pd.DataFrame:
        ...
        logger.debug("Extracted and scaled features: %s", df_scaled.to_dict(orient="records"))
        return df_scaled
    ...
    def get_prediction_from_url(self, url: str, company_name: str) -> RawScore:
        logger.debug("get_prediction_from_url called for %s (url=%s)", company_name, url)
